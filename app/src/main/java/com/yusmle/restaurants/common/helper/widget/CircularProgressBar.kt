package com.yusmle.restaurants.common.helper.widget
 ...
 *
 * @See [https://github.com/lopspower/CircularProgressBar/blob/master/circularprogressbar/src/main/java/com/mikhaellopez/circularprogressbar/CircularProgressBar.kt]
 */
class CircularProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        // Value
        progress = attributes.getFloat(R.styleable.CircularProgressBar_pb_progress, progress)
        progressMax = attributes.getFloat(
            R.styleable.CircularProgressBar_pb_progress_max, progressMax
        )

        // StrokeWidth
        progressBarWidth = attributes.getDimension(
            R.styleable.CircularProgressBar_pb_progressbar_width, progressBarWidth).pxToDp()
        backgroundProgressBarWidth = attributes.getDimension(
            R.styleable.CircularProgressBar_pb_background_progressbar_width,
            backgroundProgressBarWidth
        ).pxToDp()

        // Color
        progressBarColor = attributes.getInt(
            R.styleable.CircularProgressBar_pb_progressbar_color, progressBarColor
        )
        attributes.getColor(R.styleable.CircularProgressBar_pb_progressbar_color_start, 0)
            .also { if (it != 0) progressBarColorStart = it }
        attributes.getColor(R.styleable.CircularProgressBar_pb_progressbar_color_end, 0)
            .also { if (it != 0) progressBarColorEnd = it }
        progressBarColorDirection = attributes.getInteger(
            R.styleable.CircularProgressBar_pb_progressbar_color_direction,
            progressBarColorDirection.value
        ).toGradientDirection()
        backgroundProgressBarColor = attributes.getInt(
            R.styleable.CircularProgressBar_pb_background_progressbar_color,
            backgroundProgressBarColor
        )
        attributes.getColor(
            R.styleable.CircularProgressBar_pb_background_progressbar_color_start, 0
        ).also { if (it != 0) backgroundProgressBarColorStart = it }
        attributes.getColor(
            R.styleable.CircularProgressBar_pb_background_progressbar_color_end, 0
        ).also { if (it != 0) backgroundProgressBarColorEnd = it }
        backgroundProgressBarColorDirection = attributes.getInteger(
            R.styleable.CircularProgressBar_pb_background_progressbar_color_direction,
            backgroundProgressBarColorDirection.value
        ).toGradientDirection()

        // Progress Direction
        progressDirection = attributes.getInteger(
            R.styleable.CircularProgressBar_pb_progress_direction, progressDirection.value
        ).toProgressDirection()

        // Round Border
        roundBorder = attributes.getBoolean(
            R.styleable.CircularProgressBar_pb_round_border, roundBorder
        )

        // Angle
        startAngle = attributes.getFloat(R.styleable.CircularProgressBar_pb_start_angle, 0f)

        // Indeterminate Mode
        indeterminateMode = attributes.getBoolean(
            R.styleable.CircularProgressBar_pb_indeterminate_mode, indeterminateMode
        )
    ...
    private fun Int.toProgressDirection(): ProgressDirection =
        when (this) {
            ...
            2 -> ProgressDirection.TO_LEFT
                ...
                "This value is not supported for ProgressDirection: $this"
            )
        }
    ...
    private fun Int.toGradientDirection(): GradientDirection =
        when (this) {
            ...
            4 -> GradientDirection.BOTTOM_TO_END
                ...
                "This value is not supported for GradientDirection: $this"
            )
        }
